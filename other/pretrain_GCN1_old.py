...
orgainc_train_data, orgainc_test_loader = OrganicMetal.data_loader(file_path=config['input_organic'],columns=columns,reaction_type=config['reaction'],tensorize_fn=tensorize_with_subgraphs,batch_size=config['batch_size'],test_size=config['test_size'],)
metal_train_data, _ = OrganicMetal.data_loader(file_path='/work/u7069586/OMGNN/data/redox_metal.csv',columns=['Metal', 'Potential','Reaction'],reaction_type=config['reaction'], tensorize_fn=metal_features,batch_size=config['batch_size'],is_metal=True,)
# organic_metal_train = orgainc_train_data + metal_train_data
organic_metal_train = orgainc_train_data 
organic_metal_train_loader = DataLoader(organic_metal_train, batch_size=config['batch_size'], shuffle=False)

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
model = OrganicMetal_GNN(node_dim=config['num_features'], bond_dim=11, hidden_dim=config['num_features'], output_dim=config['output_size'], dropout=config['dropout']).to(device)
...
for epoch in range(config['num_epochs']):
    ...
    test_loss, test_reg_loss, test_cla_loss, test_accuracy  = OrganicMetal.evaluate_model(model, orgainc_test_loader, device, output_file=None)
    ...
    df_loss = pd.DataFrame({'train_loss': train_loss_history, 'test_loss': test_loss_history, 'train_reg_loss': train_reg_history, 'test_reg_loss': test_reg_history, 'train_cla_loss': train_cla_history, 'test_cla_loss': test_cla_history, 'test_cla_accuray':test_accuracy_history})
    df_loss.to_csv(os.path.join(config['save_path'], f"{config['version']}-{config['batch_size']}-{config['num_epochs']}_loss.csv"))

# evaluate model
OrganicMetal.evaluate_model(model, organic_metal_train_loader, device, output_file=f"{config['version']}-{config['reaction']}-{config['mode']}-{config['batch_size']}-{config['num_epochs']}_train_pred_true.csv")
