...
def plot_learning_curves(csv_path, output_dir, version_name, epoch_count=None):
    ...
    ax.set_title(f'Total Loss Curves{title_suffix}')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ...
    ax.set_title(f'Classification Loss Curves{title_suffix}')
    ax.set_xlabel('Epoch')
    ...
    ax.set_title(f'Regression Loss Curves{title_suffix}')
    ax.set_xlabel('Epoch')
    ...
    # RMSE曲線
    ax = axes[0]
    if 'rmse_gt' in df.columns:
        ax.plot(df['epoch'], df['rmse_gt'], 'g-', label='RMSE (All Ground Truth)')
    if 'rmse_hit' in df.columns:
        ax.plot(df['epoch'], df['rmse_hit'], 'm-', label='RMSE (Hits Only)')
    ...
    ax = axes[1]
    ax.plot(df['epoch'], df['recall'], 'r-', label='Recall')
    ...
    if epoch_count is not None:
        ...
        print(f"學習曲線已保存到 {os.path.join(output_dir, f'{version_name}_graph.png')}")
    else:
...
if __name__ == "__main__":
    ...
    csv_path = "../results/pka_model/pka_ver1_loss.csv"
    output_dir = "../results/"
    version_name = "pka_ver1"
